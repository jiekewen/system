  ...
  <div class="MessageList">
    ...
    <View-page class="Viewpage-content">
      ...
      <el-table 
        ...
        <el-table-column align="center" label="报警时间" prop="alarmtime" width="230" :formatter="formatAlarmtime" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" label="类别" prop="alarmtype" width="110" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column align="center" label="确认时间" prop="confirmtime" width="230" :formatter="formatConfirmtime" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" label="消息状态" prop="status" width="100" :formatter="formatStatus"></el-table-column>
        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
            <el-button v-if="scope.row.status === true" @click.native.prevent="deleteRow(scope.row.id)" type="text" size="small">删除</el-button>
  ...
  data() {
   ...
   methods: {
    ...
    formatConfirmtime: function (row, column) {
      if(row.confirmtime == '' || row.confirmtime == null)
        return
    ...
    timeClick(value){
      ...
      this.selectFunction();
      console.log(moment(value[0]).format('YYYY-MM-DD HH:mm:ss')+"=="+moment(value[1]).format('YYYY-MM-DD')+" 23:59:59")
    },
    //获取报警总数--分页
    getTableTotal(){
      var checkData = {
      ...
      this.$http
        .post("alarm/alarmCount", checkData)
        .then(response => {
    ...
    selectFunction(){
      ...
      //查询方法  
      this.$http
      .post("alarm/alarmList", checkData)
      .then(response => {
    ...
    switchChange(index){
      ...
      cancelButtonText:"取消"
        ...
        this.$http
        .post("alarm/updateStatus", checkData)
        .then(response => {
          if(response.status == "200"){
            if (response.data.code == 0) {
              //返回值填充页面
    ...
    deleteRow(index) {
      ...
      this.$confirm("是否确认删除此数据",'提示',{
        ...
        this.$http
        .delete('alarm/deleteAlarm?id='+index)
        .then(response => {
          if(response.status == "200"){
            if (response.data.code == 0) {
              //返回值填充页面
